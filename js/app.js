...
function announceRender(view){document.dispatchEvent(new CustomEvent("marvel:render",{detail:{view}}))}
...
function saveState({sync=true}={}){state.activeCharacter=activeCharacter;localStorage.setItem(activeStorageKey,JSON.stringify(state));if(sync&&accountView.user)queueCloudState(state);window.MarvelGlobalSearch?.refresh?.()}
...
function routeIssueById(id){return currentCharacter?.issues?.find(issue=>issue.id===id)||null}
function physicalIssueId(issueOrId){const issue=typeof issueOrId==="string"?routeIssueById(issueOrId):issueOrId;return issue?.physicalId||issue?.id||String(issueOrId||"")}
function status(id){
  const physicalId=physicalIssueId(id),item=state.collection?.[physicalId]||{};
  const editionPhysical=!!window.MarvelEditions?.get(physicalId)&&!!window.MarvelEditions?.isOwned(state,physicalId);
...
function setStatuses(ids,patch){
  ...
  for(const id of new Set(ids)){
    if(!id)continue;
    const physicalId=physicalIssueId(id),entry={...(state.collection[physicalId]||{})};
    ...
    state.collection[physicalId]=entry;
...
function issueElementId(issue){return `issue-${String(issue?.id||"").replace(/[^a-zA-Z0-9_-]/g,"-")}`}
...
function issueReadingPosition(issue,issues=requiredIssues()){
  const declared=Number(issue?.seq??issue?.readingStep?.position);
  if(Number.isFinite(declared)&&declared>0)return declared;
  const referenceIndex=issues.indexOf(issue),idIndex=issues.findIndex(candidate=>candidate.id===issue?.id),index=referenceIndex>=0?referenceIndex:idIndex;
...
function pathIconSource(path){
  const override=path?.type==="event"?"":pathIconCatalog?.paths?.[path?.id]||"";
...
function pathIconImage(path,{alt="",className="",lazy=false,onerror="this.style.display='none'"}={}){
  ...
  const classes=[className,"pathIconImage",path?.type==="event"?"pathEventCover":"",raster?"pathIconRaster":"pathIconVector"].filter(Boolean).join(" ");
...
async function loadCharacter(id){
  ...
  if(Array.isArray(c.issueSources)){const light=c,heavy=await loadEncodedCharacter(id,meta);c={...light,...heavy,archives:light.archives??heavy.archives,relatedPaths:light.relatedPaths??heavy.relatedPaths,branches:light.branches??heavy.branches}}
  c.issues??=[];
...
async function switchCharacter(id,{updateHash=true,issue=null,animate=true}={}){
  if(animate&&window.MarvelMotion?.transition)return window.MarvelMotion.transition(()=>switchCharacter(id,{updateHash,issue,animate:false}));
  ...
  if(routeTarget)requestAnimationFrame(()=>$(issueElementId(routeTarget))?.scrollIntoView({behavior:"smooth",block:"center"}));
...
async function renderHomeContinue(meta){
  ...
  try{
    ...
    const progress=state.characters?.[meta.id]?.issues||{},required=character.issues.filter(i=>i.required!==false&&!i.future),next=required.find(i=>!progress[i.id]?.read),nextPosition=next?issueReadingPosition(next,required):0,readCount=required.filter(i=>progress[i.id]?.read).length,progressPct=Math.min(100,Math.round(readCount/(required.length||1)*100));
    ...
    else els.homeContinue.innerHTML=`<div class="homeContinueHead"><span>Continua a leggere</span><span class="homeContinueCloud">${icon(accountView.user?"cloud":"book")}${accountView.user?"Sincronizzato":"Salvato in locale"}</span></div><div class="homeContinueBody" style="--continue-accent:${esc(meta.accent||"#ed1d24")}"><div class="homeContinueCover"><div class="fallback">${esc(next.name)}</div>${coverImg(next,false,meta.accent)}</div><div class="homeContinueMeta"><div class="homeContinueRoute"><div class="homeContinueHero">${pathIconImage(meta,{alt:meta.name})}<span><b>${esc(meta.name)}</b><small>${esc(meta.subtitle||"Percorso attivo")}</small></span></div><span class="homeContinueRouteBadge"><small>Prossima tappa</small><b>${nextPosition}/${required.length}</b></span></div><div class="homeContinueIssue"><span class="homeContinueIssueKicker">La tua prossima lettura</span><h2>${esc(next.name)}</h2><p class="homeContinueIssueTitle">${esc(next.title)}</p><div class="homeContinueIssueData"><span><small>Pubblicazione</small><b>${esc(next.date)}</b></span><span><small>Sezione</small><b>${esc(next.era)}</b></span></div></div><div class="homeContinueFooter"><div class="homeContinueProgress"><div class="homeContinueProgressCopy"><span>Progresso del percorso</span><b>${readCount} di ${required.length} letti</b></div><div class="homeContinueProgressTrack" role="progressbar" aria-label="Progresso ${esc(meta.name)}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${progressPct}"><span style="width:${progressPct}%"></span></div></div><button type="button" data-continue-char="${esc(meta.id)}"><span>Continua la lettura</span>${icon("arrow")}</button></div></div></div>`;
...
function showHome({updateHash=true,animate=true}={}){
  if(animate&&window.MarvelMotion?.transition)return window.MarvelMotion.transition(()=>showHome({updateHash,animate:false}));
...
function renderNext(){const i=nextIssue();if(!i){els.nextPanel.innerHTML=`<div class="nextMeta" style="grid-column:1/-1"><div class="label">Percorso completato</div><h2>Sei in pari con ${esc(currentCharacter.name)}.</h2><p>${esc(currentCharacter.end)}</p></div>`;return}const st=status(i.id),required=requiredIssues(),position=issueReadingPosition(i,required);els.nextPanel.innerHTML=`<div class="nextCover"><div class="fallback">${esc(i.name)}</div>${coverImg(i,false)}</div><div class="nextMeta"><div class="label">Leggi adesso · ${position}/${required.length}</div><h2>${esc(i.name)}</h2><p>${esc(i.date)} · ${esc(i.era)}<br>${esc(i.title)}</p><div class="nextBtns"><button type="button" class="btn formatPhysical ${st.physical?"primary":""}" id="nextPhysical">${icon(st.physical?"check":"archive")}<span>Fisico</span></button><button type="button" class="btn formatDigital ${st.digital?"primary":""}" id="nextDigital">${icon(st.digital?"check":"cloud")}<span>Digitale</span></button><button type="button" class="btn done" id="nextRead">${icon(st.read?"check":"book")}<span>${st.read?"Letto":"Segna letto"}</span></button><button type="button" class="btn" id="nextJump"><span>Mostra</span>${icon("arrow")}</button></div></div>`;$("nextPhysical").onclick=()=>setStatus(i.id,{physical:!st.physical});$("nextDigital").onclick=()=>setStatus(i.id,{digital:!st.digital});$("nextRead").onclick=()=>setStatus(i.id,{read:!st.read});$("nextJump").onclick=()=>jumpToIssue(i)}
function renderRoute(){
  const publications=(currentCharacter.series||[]).length+(currentCharacter.archives||[]).length;
  let relatedCount=0,h=(currentCharacter.series||[]).map((s,k)=>`<div class="routeCard"><b>${k+1} · ${esc(s.name)} ${esc(s.range)}</b><span>${esc(s.publisher)} · ${esc(s.years)}</span></div>`).join("");
  for(const a of currentCharacter.archives||[]){const linked=a.pathId&&manifest.characters.some(meta=>meta.id===a.pathId);h+=`${linked?`<a class="routeCard archive relation" href="#/${esc(a.pathId)}">`:'<div class="routeCard archive">'}<b>${esc(a.name)} ${esc(a.range)}</b><span>${esc(a.publisher)} · ${esc(a.years)}<br>${esc(a.status)}${linked?" · Apri percorso →":a.url?` · <a href="${esc(a.url)}" target="_blank" rel="noopener">Apri ↗</a>`:""}</span>${linked?"</a>":"</div>"}`}
  const related=[...(currentCharacter.relatedPaths||[]),...(currentMeta.relatedPaths||[]),...(currentCharacter.branches||[]).map(branch=>branch.id)],seen=new Set();
  for(const id of related){if(seen.has(id)||id===activeCharacter)continue;seen.add(id);const meta=manifest.characters.find(item=>item.id===id);if(!meta)continue;relatedCount++;const branch=(currentCharacter.branches||[]).find(item=>item.id===id);h+=`<a class="routeCard relation" href="#/${esc(id)}"><b>${esc(branch?.label||meta.name)}</b><span>${esc(branch?.relation||"Percorso collegato")} · Apri →</span></a>`}
  els.route.innerHTML=h;
  els.routeDisclosure.hidden=!h;
  els.routeDisclosureMeta.textContent=[publications?`${publications} pubblicazion${publications===1?"e":"i"}`:"",relatedCount?`${relatedCount} collegament${relatedCount===1?"o":"i"}`:""].filter(Boolean).join(" · ");
  if(els.routeDisclosure.dataset.path!==activeCharacter){els.routeDisclosure.open=false;els.routeDisclosure.dataset.path=activeCharacter}
  renderNotices();
}
function renderNotices(){const id=activeCharacter,ns=[];if(["antman","wasp","scarletwitch","vision","wonderman"].includes(id))ns.push(["Percorso personale + Vendicatori","Le tappe condivise riutilizzano lo stesso albo fisico del percorso Vendicatori: Fisico/Digitale sono globali, mentre Letto rimane indipendente per questo percorso."]);if(id==="avengers")ns.push(["Dalla vera formazione","Il percorso ora parte da Avengers #1: Loki riunisce Thor, Iron Man, Ant-Man, Wasp e Hulk. La nascita dei Vendicatori della Costa Ovest arriva prima che le due squadre inizino a interagire."],["Storie italiane spezzate","Quando una storia USA prosegue su due albi italiani, entrambe le parti sono indicate. Se lo stesso volume torna più avanti, Fisico/Digitale resta condiviso ma Letto segue la singola tappa."]);if(id==="ironman")ns.push(["Ordine narrativo","Il numero grande indica la posizione nel percorso di lettura; il titolo conserva il numero dell’edizione italiana. Gli inserti cronologici obbligatori sono evidenziati."],["Edizioni alternative","Quando una raccolta copre le storie di uno o più spillati, usa il pulsante Edizioni: la tappa risulterà recuperata senza segnare come posseduto uno spillato che non hai."]);if(id==="spiderman")ns.push(["🕷️ Editoriale Corno esclusa","Il percorso parte da L'Uomo Ragno #1 — Star Comics, maggio 1987. Le cover puntano direttamente alle schede italiane ComicsBox."],["Numerazione fino al #899","Gli albi futuri annunciati restano visibili ma non vengono conteggiati nel progresso leggibile."]);if(id==="hulk")ns.push(["💥 Devil & Hulk","Hulk e Daredevil condividono la stessa testata italiana. Fisico/Digitale sono globali per albo; Letto resta separato per personaggio."]);if(id==="hulk-classic-corno")ns.push(["Archivio storico separato","Questo percorso non precede né altera Hulk moderno: segue i capitoli USA da Hulk e i Difensori alla continuazione su L'Uomo Ragno Corno."],["Albo antologico → storia","La card mostra l'albo fisico completo e, sotto, le sole storie da leggere per questa tappa."]);if(id==="daredevil")ns.push(["Devil & Hulk condiviso","La copia fisica dell'albo vale anche per Hulk; lo stato Letto resta specifico di Daredevil e seleziona soltanto i suoi contenuti USA."]);if(id==="venom")ns.push(["Eventi collegati","Absolute Carnage e King in Black riutilizzano gli stessi albi fisici degli eventi; ogni lettura resta indipendente per percorso."]);if(id==="knull")ns.push(["Una saga, non un riassunto","Il percorso conserva l'ordine delle rivelazioni: prima l'ombra in Thor, poi il mito di Klyntar, la vera origine in Venom e infine Absolute Carnage e King in Black."],["Contenuti selezionati","Negli albi antologici leggi soltanto i capitoli indicati nella card; gli extra restano nei rispettivi percorsi Venom, Thor o Guardiani."]);if(id==="doctor-doom")ns.push(["Tre famiglie, un solo percorso","Doom è collegato a Fantastici Quattro, Mistico e Cosmico senza duplicare le edizioni condivise."]);if(id==="wolverine-616")ns.push(["Terra-616","Il percorso personale di Logan completa la famiglia X-Men e resta distinto dalle versioni Ultimate."]);if(id==="thor")ns.push(["⚡ Salto intenzionale #78–109","Nel percorso personale di Thor questi numeri sono facoltativi; la testata ospita soprattutto Nuovi Vendicatori/Capitan America."]);if(id==="cap")ns.push(["🛡️ Ingresso moderno","Il percorso parte dal blocco antologico italiano legato alla gestione moderna e poi passa alla testata Capitan America."]);els.noticeWrap.innerHTML=ns.map(([b,p])=>`<div class="notice"><b>${b}</b><p>${p}</p></div>`).join("")}
...
function renderFilters(){
  ...
  const source=currentCharacter.issues.filter(i=>(optionalVisible||!i.skip)&&(activeSeries==="Tutte"||i.seriesId===activeSeries)&&(!q||issueSearchText(i).includes(q)));
...
function issueContentHtml(i){
  const contents=Array.isArray(i.contents)?i.contents:[],selectedIds=new Set(i.readingStep?.contentIds||[]),selected=contents.filter(content=>selectedIds.has(content.id));
  if(!selected.length)return "";
  const labels=selected.map(content=>content.title||`${content.series} #${content.number}`);
  const count=contents.length>selected.length?`<span>${selected.length} di ${contents.length} contenuti dell'albo</span>`:`<span>${contents.length} contenut${contents.length===1?"o":"i"} USA</span>`;
  return `<div class="contentMap"><b>In questa tappa</b><span class="contentMapTitles">${esc(labels.join(" · "))}</span>${count}</div>`;
}
function issueHtml(i){
  ...
  const contentBadge=i.readingStep?.contentIds?.length?'<span class="contentBadge">Contenuti selezionati</span>':"";
...
function jumpToIssue(i){history.replaceState(null,"",`#/${activeCharacter}/${routeIssueToken(i)}`);$(issueElementId(i))?.scrollIntoView({behavior:"smooth",block:"center"})}
function renderAll(){if(!currentCharacter)return;renderCharacters();renderHero();renderStats();renderNext();renderRoute();renderSeriesNav();renderFilters();renderBlocks();els.showOptional.style.display=currentCharacter.issues.some(i=>i.skip)?"block":"none";els.showOptional.textContent=optionalVisible?"− Nascondi numeri facoltativi":"+ Mostra numeri facoltativi";announceRender("tracker")}
