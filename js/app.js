...
function renderRoute(){
  let h=(currentCharacter.series||[]).map((s,k)=>`<div class="routeCard"><b>${k+1} · ${esc(s.name)} ${esc(s.range)}</b><span>${esc(s.publisher)} · ${esc(s.years)}</span></div>`).join("");
  ...
  for(const id of related){if(seen.has(id)||id===activeCharacter)continue;seen.add(id);const meta=manifest.characters.find(item=>item.id===id);if(!meta)continue;const branch=(currentCharacter.branches||[]).find(item=>item.id===id);h+=`<a class="routeCard relation" href="#/${esc(id)}"><b>${esc(branch?.label||meta.name)}</b><span>${esc(branch?.relation||"Percorso collegato")} · Apri →</span></a>`}
  els.route.innerHTML=h;renderNotices();
}
