...
async function renderHomeContinue(meta){
  ...
  try{
    ...
    const progress=state.characters?.[meta.id]?.issues||{},required=character.issues.filter(i=>i.required!==false&&!i.future),next=required.find(i=>!progress[i.id]?.read),readCount=required.filter(i=>progress[i.id]?.read).length,progressPct=Math.min(100,Math.round(readCount/(required.length||1)*100));
