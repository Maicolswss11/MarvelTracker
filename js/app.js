...
const els = {
  ...
  homeView: $("homeView"), trackerView: $("trackerView"), homeBtn: $("homeBtn"), homeTopResume: $("homeTopResume"), homeResume: $("homeResume"), homeExplore: $("homeExplore"), homeHeroIcons: $("homeHeroIcons"), homeStats: $("homeStats"), homeCharacterGrid: $("homeCharacterGrid"), homeCharactersSection: $("homeCharactersSection")
};
...
function renderHome(){
  ...
  els.homeHeroIcons.innerHTML=manifest.characters.map(c=>`<span class="homeHeroIcon" style="--hero-accent:${esc(c.accent)}" title="${esc(c.name)}"><img src="${esc(versioned(c.logo))}" alt="" onerror="this.parentElement.style.display='none'"></span>`).join("");
  ...
  els.homeCharacterGrid.innerHTML=manifest.characters.map(c=>{const read=readCountFor(c.id),total=c.totalRequired||0,pct=Math.min(100,Math.round(read/(total||1)*100));return `<button type="button" class="homeCharCard" style="--hero-accent:${esc(c.accent)}" data-home-char="${esc(c.id)}" aria-label="Apri il percorso di ${esc(c.name)}, ${read} albi letti su ${total}"><img class="homeCharLogo" src="${esc(versioned(c.logo))}" alt="Logo ${esc(c.name)}" onerror="this.style.visibility='hidden'"><span class="homeCharMeta"><b>${esc(c.name)}</b><span>${esc(c.subtitle)}</span></span><span class="homeCharProgress"><span class="homeCharProgressTop"><span>Progresso</span><b>${read}/${total}</b></span><span class="homeCharTrack"><span style="width:${pct}%"></span></span><span class="homeCharOpen">Apri percorso ${icon("arrow")}</span></span></button>`}).join("");
...
(async()=>{try{await loadManifest();const h=parseHash();if(h.view==="home"){showHome({updateHash:false});if(!location.hash)history.replaceState(null,"","#/home")}else await switchCharacter(h.character,{updateHash:false,issue:h.issue})}catch(e){console.error(e);els.seriesBlocks.innerHTML=`<div class="loading error"><b>Errore di caricamento</b><br>${esc(e.message)}<br><br>Apri il sito tramite GitHub Pages o un server HTTP: i JSON non possono essere caricati correttamente con alcuni browser da file://.</div>`}})();
